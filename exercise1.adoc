...
+
Building images that are customized is great, but sometimes you also need to provide information to the container when you run it. For example: if you review the output on the console from Step 6, we can see that the container that Red Hat provides for JBoss EAP uses 1.3 GB of memory (look for -Xmx and -Xmx). This might be a bit high for a development environment and to low for a production environment. We can alter this by passing environment variables to the container that override the defaults. In this step we will override the DEFAULT jvm setting, but optimizing it for low memory and quick startup.
+
