...
----
+
The RHEL container, provided by Red Hat, is on purpose kept very minimal, the idea is that each service based on a container will define and install the it's own dependencies. Tools like ps (used to list running processes) is not installed in the RHEL container, but can easily be installed using `yum`.
+
IMPORTANT: Installing packages and changing configurations using the interactive console is strongly discouraged, since maintaining containers that requires manual setup is much harder than maintaining containers that are automated.
+
...
[source,bash,numbered]
----
$ docker run -d -p 8081:8080 jboss-eap-6/eap:6.4
...
----
+
The container running JBoss EAP is started and port 8080 in the container is mapped to 8081 on your host. Verify that the server responds by opening a browser to http://localhost:8081
+
NOTE: Using -P option all the exposed ports (for JBoss EAP 8080) are automatically mapped to an available port on the server.
+
...
<4> Expose Web, Admin Web and Remote Admin API.
+
...
[source,bash]
----
$ docker run -it -p 8081:8080 -p 9990:9990 -p 9999:9999 <your-alias>/jboss-eap:1.0
----
+
Try login in to the administation control by access http://localhost:9990 using admin/admin-123 as username/password.
+
Stop the container using `CTRL-C`

Step 7 - Environment variables::
+
Building images that are customized is great, but sometimes you also need to interact with the container to add runtime behavior. For example: Currently the container that Red Hat provides for JBoss EAP uses 1.3 GB of memory. This might be a bit much in a development environment and to low in a production environment. We can do this by passing environment variables to the container overriding certain environmental settings.
+
Review the `jboss-dev.env` file in the `lab` directory. Record the values -Xms and -Xmx.
+
Start the container like this:
+
[source,bash]
----
docker run -it -p 9990:9990 --env-file=jboss-dev.env tqvarnst/jboss-eap:1.0
----
+
