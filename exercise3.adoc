...
----
+
Visit <http://localhost:8081/ticket-monster in your web browser and use the application to verify it deployed correctly.
+
...
### Bundling the application the container
...
+
----
docker run -i -p 8081:8080 -p 9990:9990 -p 9999:9999 --link ticket-monster-db:postgres <your-alias>/jboss-eap-postgres-ticket-monster:1.0
----
+
Visit <http://localhost:8081/ticket-monster in your web browser and use the application to verify it deployed correctly.
...
Step 1 - Create the application build::

This time we'll start with the base `lab/base` image we created for you - it container Java, Maven and Git. It looks like:
----
FROM rhel7.1
...
+
----
docker build -t <your-alias>/ticket-monster-postgres:1.0 -f ticket-monster-build/Dockerfile .
----

Step 2 - Use the application build::
...
docker -v target:/target run <your-alias>/ticket-monster-postgres:1.0
----
+
