...
We're going to use the postgres container from the docker registry.
+
...
We'll use the `psql` command to interrogate the PostgreSQL container, and make sure it's working. Psql isn't installed on your workstation; instead, we'll use another docker container to run it, linking to the Postgres container.
+
...
----
+
So you can see there are POSTGRES_* environment variables providing plenty of information about source container.
+
...
----
+
When asked for a assword enter `UsW4fznqLmGRh6`. You should now see the psql console:
----
$ docker run -it --link ticket-monster-db:postgres --rm postgres:9.4 sh -c 'exec psql -h "$POSTGRES_PORT_5432_TCP_ADDR" -p "$POSTGRES_PORT_5432_TCP_PORT" -U postgres'
...
Step 4 - Include your application in the Docker container::

During development it's handy to deploy

Step 5 - Start the application server and verify data is stored in Postgres::
